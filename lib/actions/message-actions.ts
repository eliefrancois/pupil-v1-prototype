...
function detectContactInfo(text: string): boolean {
  return PHONE_REGEX.test(text) || EMAIL_REGEX.test(text) || SOCIAL_REGEX.test(text)
}
...
- Attempts to share personal contact info (phone, email, social media handles) to move communication off-platform.
...
Also determine if the message contains REAL personal contact information.
False positives to ignore: example numbers like "(555) 555-5555", area code mentions, email format explanations, @mentions that aren't real handles.
...
export async function sendMessage(input: {
  ...
  if (moderation.is_flagged) {
    ...
    // 3+ strikes → flag account for review
    if (currentStrikes >= 3) {
      await supabase
        .from('users')
